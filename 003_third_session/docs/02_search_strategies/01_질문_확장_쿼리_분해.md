...
_PROMPT = dedent(
    ...
    Decompose the user's question into 2–4 focused sub-queries that can be searched independently.
    Rules:
    - Keep each sub-query short and specific.
    - Preserve the user's language.
    - Avoid yes/no questions; make them searchable.
    - If the question is already simple, return a single item with the original question.
    Output format:
    - One sub-query per line, prefixed with "- ".
    Question: {{question}}
    """
).strip()
_DECOMPOSE_PROMPT = PromptTemplate(_PROMPT)
...
class QueryDecomposeGraph:
    ...
    def __init__(self, retriever: Any) -> None:
        ...
        self._graph.add_node("search", lambda s: node_search(s, self._retriever))
