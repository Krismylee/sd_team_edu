...
# 1) 쿼리 분해 프롬프트
_DECOMPOSE_PROMPT = PromptTemplate(
    dedent(
        """
        You are a query decomposition expert.
        Decompose the user's question into 2–4 focused sub-queries.
        Output format:
        - One sub-query per line, prefixed with "- ".
        Question: {question}
        """
    ).strip()
)
...
# 2) 관련 문서 인덱스 선택 프롬프트
_SELECT_PROMPT = PromptTemplate(
    dedent(
        """
        You are selecting relevant documents for the user's question.
        You will receive numbered documents.
        Return only the indices of relevant documents as a comma-separated list.
...
_HYDE_PROMPT = PromptTemplate(
    dedent(
        ...
        Write a concise document that would answer the question.
        Question: {question}
        """
    ).strip()
)
...
async def decompose_query(question: str) -> list[str]:
    ...
    text = (_DECOMPOSE_PROMPT | llm | StrOutputParser()).invoke({"question": question})
...
async def select_relevant_docs(question: str, docs: list[dict]) -> list[dict]:
    ...
    text = (_SELECT_PROMPT | llm | StrOutputParser()).invoke(
...
async def adaptive_hyde_search(retriever: Any, question: str) -> list[dict]:
    """적응형 HyDE: 관련 문서가 없을 때만 수행한다."""
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
    hyde_doc = (_HYDE_PROMPT | llm | StrOutputParser()).invoke({"question": question})
    emb = OpenAIEmbeddings(model="text-embedding-3-small")
    vec = emb.embed_query(hyde_doc)
    if retriever is None:
        return [{"content": hyde_doc, "metadata": {"hyde": True}}]
    return retriever.similarity_search_by_vector(vec, k=5)
...
async def answer_with_evidence(question: str, docs: list[dict]) -> str:
    ...
    return (_ANSWER_PROMPT | llm | StrOutputParser()).invoke(
        {"question": question, "evidence": evidence}
    )


async def stream_answer(answer: str) -> Iterable[str]:
    """답변을 스트리밍한다고 가정한 제너레이터(개념)."""
    for token in answer.split():
        yield token


async def stream_sources(docs: list[dict]) -> Iterable[str]:
...
async def node_select(state: dict) -> dict:
    ...
    filtered = []
    for docs in state["search_results"]:
        selected = await select_relevant_docs(question, docs)
...
async def node_hyde(state: dict) -> dict:
    ...
    retriever = state["retriever"]
    fixed = []
    for docs in state["filtered_results"]:
        if docs:
            fixed.append(docs)
        else:
            fixed.append(await adaptive_hyde_search(retriever, question))
...
async def node_answer(state: dict) -> dict:
    ...
    answers = []
    evidence_docs = []
    for docs in state["final_docs"]:
        deduped = dedupe_docs(docs)
        evidence_docs.extend(deduped)
        answers.append(await answer_with_evidence(state["question"], deduped))
