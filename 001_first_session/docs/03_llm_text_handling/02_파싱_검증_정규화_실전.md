...
```python
# 파일: src/examples/regex_answer_parser.py
"""
목적: 정규식으로 핵심 값을 추출한다.
...
```python
# 파일: src/examples/chain_with_regex_parser.py
"""
...
class SentimentPromptSingleton:
    """감정 분류 프롬프트 싱글턴."""
...
prompt = SentimentPromptSingleton.get_template()
raw_output = "감정=긍정\n이유=가격이 좋음"
