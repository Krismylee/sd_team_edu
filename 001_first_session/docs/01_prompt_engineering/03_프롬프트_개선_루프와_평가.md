...
설계 패턴은 **레지스트리(Registry) + 싱글턴(Singleton)**에 가깝습니다.

```python
# 파일: src/examples/prompt_version_registry.py
"""
목적: 프롬프트 버전을 명시적으로 관리한다.
설명: 버전별 템플릿을 등록하고 필요한 버전을 선택한다.
디자인 패턴: Registry + Singleton
...
template_v1 = PromptTemplate.from_template(prompt_v1)
template_v2 = PromptTemplate.from_template(prompt_v2)
...
```python
# 파일: src/examples/prompt_ab_compare.py
"""
